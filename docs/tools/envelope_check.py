...
Note this file is both a mirror and the enforcer of GENERATOR_BRANCHES below. That is
intentional -- it is a model side like any other, and exempting it would leave the gap in the
one file most likely to be edited when a branch changes.
"""
...
# dispersion_mc.py samples the model rather than integrating it, and it does NOT read either
# enable flag -- a grep for enableSkillVariance/enablePassionVariance in that file returns
# nothing. Declaring the marker there anyway would make this table certify a mirror that does
# not exist, which is the exact failure it is built to prevent. The exemption is NARROW and
# named rather than a blanket skip: every other branch is still required of that file, and if
...
GENERATOR_BRANCHES = (
    ...
    ("passion-capacity", "PassionVarianceApplier.cs:115,164 (one passion per eligible skill)",
     MIRROR_SITES),
    ("enable-toggles", "HarmonyPatches.cs:58-60, GrowUpVariance.cs:97-99", TOGGLE_MIRROR_SITES),
    ("skill-clamp", "SkillVarianceApplier.cs:73-74 (RoundToInt then Clamp 0..20)", MIRROR_SITES),
)
...
def check_mirror_markers():
    ...
    if missing:
        ...
        total = sum(len(sites) for _, _, sites in GENERATOR_BRANCHES)
        print(f"generator/mirror checklist: {len(GENERATOR_BRANCHES)} branches, "
              f"{total} declarations, all present")
    return len(missing)

GEN_HEADER = """// <auto-generated>
//     Regenerated by {tool}. DO NOT EDIT BY HAND.
//
//     These are the REFERENCE Best-of-N figures, integrated on a {qgrid}q x {xgrid}x dispersion
//     grid. The mod ships its own integrator (DispersionModel.BestOfN, reached through
//     PawnVarianceSettings.CalculateBestOfNScoreCore) because custom profiles need a live figure
//     that no precomputed table can cover -- so there are genuinely two implementations of the
//     same integral, and they must agree.
//
//     The two sides run the SAME grid: DispersionModel.QNodes/XNodes equal this tool's
//     QGRID/XGRID exactly. This header used to claim a 20000-node reference against a 1024-node
//     integrator; both numbers belonged to the retired analytic scheme and the resolution gap they
//     implied no longer exists (audit finding Q-09). The expected raw disagreement is
//     float-precision-scale, not the ~0.9% that framing predicted.
//
...
{{
    ...
    {{
        ...
        // The grid these figures were integrated on. DispersionModel.QNodes/XNodes must match --
        // that is what makes "two implementations of one integral" checkable rather than assumed.
        public const int ReferenceQNodes = {qgrid};
        public const int ReferenceXNodes = {xgrid};
...
def write_generated(C, P, score, order):
    ...
    text = GEN_HEADER.format(
        tool="docs/tools/envelope_check.py", qgrid=QGRID, xgrid=XGRID,
...
# Node count for the RETIRED analytic scheme below (beta_grid / expected_best_of_n), which now
# serves only main()'s zero-noise self-check. No shipped figure is integrated on this grid -- the
# printed table and EnvelopeFigures.g.cs both come from make_grid_score at QGRID x XGRID.
GRID = 20000
