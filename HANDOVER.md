...
|---|---|
| **A race with no concrete `PawnKindDef` is not offered in the race Add menu.** `SelectableRaces()` traverses kind defs, so a humanlike race whose only def is an abstract base with zero concrete children — or one created purely in code — never reaches the list. | Reaching such a race would need a second source, and that source is **all humanlike `ThingDef`s**, which would re-admit every abstract and unreferenced race def the current filter deliberately drops. **The filter stays as specified.** See *The Add-menu filter has two halves* for the two filters and why both are load-bearing. |
...
- **`Humanlike`** excludes the ~37 mechanoid alien races HAR mods ship (`Wolfein_Mechanoid_*`,
  `*_Mechanoid_*`, `*_Drone*`, `*_FloatUnit_*`). Without it the menu is unusable.
